...
class SleepChartView: UIViewController, iCarouselDataSource, iCarouselDelegate {
    ...
    override func viewDidLoad() {
        ...
        if var plistFile = PlistFile(named: "SleepData") {
            ...
            if let retrievedData : [String : Double] = (PlistFile(named: "SleepData")?.dictionary) as! [String : Double] {
                for index in 1...10 {
                    data.append(retrievedData["May " + String(index)]!)
                    labels.append("May " + String(index))
                }
                graphView.set(data: data, withLabels: labels)
                items.append(String(getAverage(dict: dict)))
                items.append(String(getSuggested(dict: dict)))
    ...
    func carousel(_ carousel: iCarousel, viewForItemAt index: Int, reusing view: UIView?) -> UIView {
        ...
        if let view = view as? UIImageView {
            ...
            theView.backgroundColor = UIColor.darkGray
